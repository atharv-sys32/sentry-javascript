...
- (Remix) The `@sentry/remix/loader` entry point was removed (same reason as `@sentry/node/loader` above). Use `node --import @sentry/remix/import` instead.
- (TanStack Start) The `@sentry/tanstackstart-react/loader` entry point was removed (same reason as `@sentry/node/loader` above). Use `node --import @sentry/tanstackstart-react/import` instead.
